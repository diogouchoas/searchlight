...
func TryUpdateSearchlightPlugin(c cs.MonitoringV1alpha1Interface, meta metav1.ObjectMeta, transform func(*api.SearchlightPlugin) *api.SearchlightPlugin) (result *api.SearchlightPlugin, err error) {
	...
	err = wait.PollImmediate(kutil.RetryInterval, kutil.RetryTimeout, func() (bool, error) {
		...
		glog.Errorf("Attempt %d failed to update SearchlightPlugin %s due to %v.", attempt, cur.Name, e2)
		return false, nil
	})

	if err != nil {
		err = fmt.Errorf("failed to update SearchlightPlugin %s after %d attempts due to %v", meta.Name, attempt, err)
	}
	return
}
